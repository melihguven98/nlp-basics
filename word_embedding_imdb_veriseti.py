...
# Note: simple_preprocess islemini fonksiyon icinde return oncesi yapabilirdik.
# Amacimiz iki islemi ayirmak

# %%

# word2vec modeli tanimla
model = Word2Vec(sentences=tokenized_documents, vector_size=50, window=5, min_count=1, sg=0)
word_vectors = model.wv # vektorlestirme

words = list(word_vectors.index_to_key)[:500]
vectors = [word_vectors[word] for word in words]

# clustering KMeans K=2
kmeans = KMeans(n_clusters=2) # KMeans: kumeleme algoritmasi
kmeans.fit(vectors)
clusters = kmeans.labels_ # 0,1 etiket
